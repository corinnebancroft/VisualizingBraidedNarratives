...
for _, row in df.iterrows():
    ...
    ax.plot(x_base, y_base, 'o', color=narrator_color, markersize=5)
    ...
    for i, char in enumerate(characters):
        ...
        ax.plot(x_char, y_char, 'o', color=color_map[char], markersize=2.5)
        ax.plot([x_base, x_char], [y_base, y_char], '-', color=color_map[char], linewidth=0.5)
    ...
    if pd.notnull(row['Start Date TT']) and pd.notnull(row['End Date TT']) and \
       pd.notnull(row['Start Page TT']) and pd.notnull(row['End Page TT']):
        ...
        if x0 != x1 and min(x0, x1) <= x_event <= max(x0, x1):
            ...
            ax.plot([x_event, x_event], [y_event, y_interp], '--', color=color, linewidth=0.25)
