...
df_chars = pd.read_csv('tt/dump/narratives_tt_characters.csv')
df_conts = pd.read_csv('tt/dump/narratives_tt_containers.csv')
df_rels = pd.read_csv('tt/dump/narratives_tt_relationships.csv')
...
date = datetime.now().strftime("%Y%m%d-%HH%MM")
df_no_duplicates.to_csv(f'data/tt/Mar29Take2/tt_StorySpaceEdges_{date}.csv', index=False)
...
date = datetime.now().strftime("%Y%m%d-%HH%MM")
final_human_df.to_csv(f'data/tt/Mar29Take2/ttTextSpaceEdges_{date}.csv', index=False)
...
output_file = 'data/tt/Mar29Take2/ttExchangesOnlyMar29.csv'
