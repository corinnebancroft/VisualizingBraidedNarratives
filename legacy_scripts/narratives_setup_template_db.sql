...
GRANT USAGE, SELECT, LOCK TABLES, SHOW VIEW ON `DB_NAME`.* TO `READER_USER`@`%`; -- also needs LOCK TABLES
GRANT USAGE, SELECT, INSERT, UPDATE, DELETE, INDEX, CREATE TEMPORARY TABLES, CREATE VIEW, SHOW VIEW ON `DB_NAME`.* TO `ADMIN_USER`@`%`;
GRANT GRANT OPTION ON `DB_NAME`.* TO 'ADMIN_USER'@'%';