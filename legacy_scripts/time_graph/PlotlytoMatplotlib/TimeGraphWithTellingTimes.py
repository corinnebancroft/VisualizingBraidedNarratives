...
df['Wrapped Evidence'] = df['Evidence'].apply(lambda x: wrap_text(x, width=50))
...
# Create hover text for event points
df['Hover Info'] = (
    "Event: " + df['Event Name'].astype(str) + "<br>" +
    "Date: " + df.apply(
        lambda row: (
            "Approximately " if str(row.get('Is Approximate?', '')).strip().upper() in ['T', 'TRUE'] else ""
        ) + row['Start Date'].strftime('%Y-%m-%d') + " - " + row['End Date'].strftime('%Y-%m-%d'),
        axis=1
    ) + "<br>" +
    "Narrator: " + df['Narrator'].astype(str) + "<br>" +
    "Page: " + df['Start Page'].astype(str) + "<br>" +
    "Characters: " + df['Participating Characters'].apply(lambda x: ', '.join(x) if x else 'Other') + "<br>" +
    "Evidence: " + df['Wrapped Evidence']
)
...
for _, row in df.iterrows():
    ...
    for i, char in enumerate(characters):
        ...
        character_traces.append(go.Scatter(
            ...
            hovertemplate=f"{char}<extra></extra>",
...
# Buttons
# Identify trace indices
narrator_indices = [i for i, trace in enumerate(fig.data) if str(getattr(trace, 'legendgroup', '')).startswith('narrator-')]
character_indices = [i for i, trace in enumerate(fig.data) if str(getattr(trace, 'legendgroup', '')).startswith('character-')]
guideline_indices = [i for i, trace in enumerate(fig.data) if getattr(trace, 'name', '') == 'guideline']

# Add toggle buttons stacked in bottom-right
fig.update_layout(
    updatemenus=[
        dict(
            type="buttons",
            ...
            buttons=[
                dict(
                    label="Toggle Narrators",
                    method="restyle",
                    args=[
                        {"visible": [True if i not in narrator_indices else "legendonly" for i in range(len(fig.data))]}
                    ],
                    args2=[
                ...
                dict(
                    label="Toggle Characters",
                    method="restyle",
                    args=[
                        {"visible": [("legendonly" if i in character_indices else True) for i in range(len(fig.data))]}
                    ],
                    args2=[
                        {"visible": [True for _ in range(len(fig.data))]}
                    ]
                ...
                dict(
                    label="Toggle Telling Time",
                    method="restyle",
                    args=[
                        {"visible": [True if i not in guideline_indices else "legendonly" for i in range(len(fig.data))]}
                    ],
                    args2=[
                        {"visible": [True for _ in range(len(fig.data))]}
                    ]
