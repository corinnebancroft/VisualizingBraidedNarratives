...
for (i, j, data, is_approx, start_date, end_date, start_page, end_page, narrator) in zip(x, y_num, pie_data,
    ...
    # Check if the pie chart has only one slice
    if sum(data) == 1:
        wedges, texts = ax_pie.pie(data, wedgeprops=dict(linewidth=2, edgecolor=narrator_colors[narrator], linestyle='solid'))
        for wedge in wedges:
            wedge.set_edgecolor(narrator_colors[narrator])
            wedge.set_linewidth(2)  # Adjust the thickness here (default is 1)
            wedge.set_linestyle('solid')  # Ensure the border is solid
    else:
        wedges, texts = ax_pie.pie(data, wedgeprops=dict(linewidth=2, edgecolor=narrator_colors[narrator]))
